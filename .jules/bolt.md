...
## 2026-07-24 - Memoize MessageList to prevent re-renders
**Learning:** Heavy inner components that map over large arrays (like MessageList) must be explicitly memoized if their parent handles high-frequency state updates (like typing indicators or realtime presence). Even if the internal map loop uses `useMemo`, the outer component function itself will re-execute on every parent render, leading to unnecessary React reconciliation checks. Furthermore, for `React.memo` to be effective, all function props passed from the parent (like `handleToggleReaction` or `handleReferenceClick`) must be stabilized using `useCallback`.
**Action:** Always verify if a child component mapping over a large dataset is receiving unstable function props from a frequently updating parent. Wrap the child in `React.memo` and stabilize the parent's callback props.
