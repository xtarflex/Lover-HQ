...
## 2024-05-24 - Array finding inside Render loops
**Learning:** Re-evaluating O(n) lookups inside rendering loops (e.g. `Array.find` or `Array.filter` on a constant array on every keystroke/frame) can introduce subtle performance issues and consume CPU unnecessarily.
**Action:** When searching constant arrays (like Emoji definitions or specific tags), use a Map (`Map<ID, Value>`) to pre-cache the array indices. Map get lookups are O(1) resulting in better performance in large loops and re-renders.
