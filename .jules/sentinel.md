...
## 2024-08-06 - XSS vulnerability via unsafe URL protocol handling
**Vulnerability:** `window.open` and `href` attributes accepted unsanitized user-provided URLs.
**Learning:** React escapes attributes but does not sanitize protocols, allowing malicious `javascript:` URIs to execute arbitrary code. Unvalidated URLs in `window.open` are similarly vulnerable.
**Prevention:** Always sanitize user-provided URLs before using them in `href` attributes or `window.open()` calls. Enforce an allowlist of safe protocols (e.g., `http:`, `https:`) using the URL API.
