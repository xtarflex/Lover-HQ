...
## 2025-02-27 - XSS via Unsafe Protocols
**Vulnerability:** User-provided URLs were directly passed into `window.open` and `href` attributes without protocol validation.
**Learning:** React escapes attributes but does not sanitize protocols. Malicious protocols like `javascript:` or `vbscript:` can lead to XSS attacks.
**Prevention:** Always validate URL protocols using a safe allowlist (e.g., `http:`, `https:`, `blob:`, `mailto:`, `tel:`) using the `URL` constructor before rendering them.
