...
## 2025-02-14 - Prevent XSS in user-provided URLs in React `href` attributes
**Vulnerability:** Found a Cross-Site Scripting (XSS) vulnerability in `MessageList.jsx` where a user-provided `media_url` for a location message was directly passed to an `<a>` tag's `href` attribute without sanitizing the protocol. This could allow an attacker to execute malicious scripts via `javascript:` protocols.
**Learning:** While React escapes HTML attributes to prevent basic XSS, it does not sanitize the protocol of URLs passed to `href` attributes. A malicious user can still inject a `javascript:` payload into a link. Parsing URLs with `new URL()` handles parsing validation but does not inherently enforce safe protocols. A base URL (e.g. `window.location.origin`) must be used for `new URL(url)` to prevent `TypeError` exceptions for valid relative paths.
**Prevention:** Always enforce an allowlist of safe protocols (e.g., `http:`, `https:`) for user-provided URLs rendered in `href` attributes. Create a helper function outside the component to parse and validate the URL, and securely fallback (e.g., to `'#'`) if the parsing fails or an unsafe protocol is used.
