...
## 2024-05-24 - [Fix XSS vulnerability in URL rendering]
**Vulnerability:** User-provided URLs passed directly to `window.open()` and `href` attributes in React without sanitization.
**Learning:** React escapes attributes but does not sanitize protocols, making it vulnerable to XSS via malicious protocols like `javascript:`.
**Prevention:** Always enforce an allowlist of safe protocols (e.g., `http:`, `https:`, `blob:`, `data:`, `mailto:`, `tel:`) when rendering user-provided URLs in React components.
