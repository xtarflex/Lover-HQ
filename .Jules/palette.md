...
## 2024-08-29 - Accessible Custom Dropdowns
**Learning:** Custom dropdown components built with generic `<button>` and `<div>` elements lack inherent semantic meaning for screen readers, unlike native `<select>` elements.
**Action:** When building custom select/dropdown components, always ensure proper screen reader support by adding `aria-haspopup="listbox"` and `aria-expanded` to the trigger button, `role="listbox"` to the panel container, and `role="option"` with `aria-selected` to each option element.
