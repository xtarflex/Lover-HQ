...
## 2024-11-20 - Adding ARIA labels to dynamically rendered emoji buttons
**Learning:** Emoji characters by themselves are not always reliably parsed by screen readers in interactive components like reaction menus. It's critical to provide explicit text equivalents (e.g. `aria-label="React with 😊"`).
**Action:** Always wrap emoji-only interactive elements in explicit, descriptive `aria-label` attributes.
