...
## 2026-08-28 - Add ARIA Roles to Custom Dropdowns
**Learning:** When building custom select/dropdown components (such as those using nested div and button elements instead of native <select> tags), always ensure proper screen reader support by adding aria-haspopup="listbox" and aria-expanded to the trigger button, role="listbox" to the panel container, and role="option" with aria-selected to each option element.
**Action:** Verify custom dropdowns for missing ARIA attributes and apply standard listbox patterns.
