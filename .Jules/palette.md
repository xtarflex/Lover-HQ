...
## 2026-09-04 - Accessible Custom Dropdowns
**Learning:** Custom dropdown components built with divs and buttons lack native select accessibility, making them invisible or confusing to screen readers.
**Action:** Always add `aria-haspopup="listbox"` and `aria-expanded` to trigger buttons, `role="listbox"` to the panel container, and `role="option"` with `aria-selected` to each option.
