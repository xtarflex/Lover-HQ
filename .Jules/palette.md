...
## 2024-07-26 - Custom Dropdown Screen Reader Support
**Learning:** Custom select/dropdown components (like `GlassDropdown.jsx`) built with nested `div` and `button` elements are completely opaque to screen readers by default. They need explicit ARIA roles to be announced correctly.
**Action:** When building or auditing custom dropdowns, always add `aria-haspopup="listbox"` and `aria-expanded={isOpen}` to the trigger button, `role="listbox"` to the panel container, and `role="option"` with `aria-selected={isSelected}` to each option element.
