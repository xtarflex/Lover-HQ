...
## 2024-08-04 - Custom Dropdown Accessibility
**Learning:** Custom select/dropdown components (such as those using nested div and button elements instead of native <select> tags) are completely opaque to screen readers by default. They require a specific combination of ARIA attributes to be interpreted as a select menu.
**Action:** Always ensure proper screen reader support for custom dropdowns by adding aria-haspopup="listbox" and aria-expanded to the trigger button, role="listbox" to the panel container, and role="option" with aria-selected to each option element.
