...
## 2026-08-20 - Accessible Custom Selects
**Learning:** When building custom select/dropdown components, always ensure proper screen reader support by adding `aria-haspopup="listbox"` and `aria-expanded` to the trigger button, `role="listbox"` to the panel container, and `role="option"` with `aria-selected` to each option element.
**Action:** Add these attributes to custom select elements to ensure accessibility.
