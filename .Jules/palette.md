...
## 2026-08-01 - Add accessibility attributes to GlassDropdown\n**Learning:** Custom select/dropdown components using nested div and button elements must have aria attributes (role="listbox", role="option", aria-selected, aria-expanded, aria-haspopup) to be accessible to screen readers.\n**Action:** Always verify custom select components have proper ARIA attributes to mimic native select behavior for screen readers.
