...
## 2026-09-17 - Add ARIA roles to GlassDropdown
**Learning:** Custom dropdown components built with generic elements (like `div` or `button` collections) are not natively understood as dropdowns by screen readers. Without proper ARIA roles, users won't know the component's state or options.
**Action:** Always add `role="listbox"` to the dropdown container, `role="option"` and `aria-selected` to the individual items, and `aria-haspopup="listbox"` with `aria-expanded` to the trigger button.
