...
## 2024-09-11 - Accessible Custom Select Components
**Learning:** When building custom select/dropdown components (such as those using nested `div` and `button` elements instead of native `<select>` tags), they are invisible as lists/options to screen readers without ARIA attributes.
**Action:** Always ensure proper screen reader support by adding `aria-haspopup="listbox"` and `aria-expanded` to the trigger button, `role="listbox"` to the panel container, and `role="option"` with `aria-selected` to each option element.
