...
## 2026-08-17 - Added missing aria-labels to emoji reaction buttons
**Learning:** Interactive emoji buttons mapped from an array without descriptive text are inaccessible to screen readers, which might just announce the character or nothing at all.
**Action:** Ensure dynamically mapped icon or emoji buttons include descriptive `aria-label` attributes, using conditional logic (e.g., `didIReact ? 'Remove reaction' : 'React with'`) if their state changes.
