...
## 2024-07-25 - Custom Dropdown Screen Reader Support
**Learning:** Custom dropdown menus built with `div` and `button` elements instead of native `<select>` elements are completely opaque to screen readers by default. The trigger button and the option list must be explicitly linked and semantically identified.
**Action:** When building custom select/dropdown components, always add `aria-haspopup="listbox"` and `aria-expanded` to the trigger button, `role="listbox"` to the panel container, and `role="option"` with `aria-selected` to each option element.
