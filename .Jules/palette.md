...
## 2026-08-30 - Context-aware aria-labels for toggles
**Learning:** Emoji reaction buttons act as toggles but lack inherent semantics for screen readers. Using a generic label is insufficient when the action changes (add vs remove).
**Action:** When implementing icon-only toggle buttons (like emoji reactions), dynamically update the `aria-label` based on state (e.g., `Remove reaction ${emoji}` vs `React with ${emoji}`) to ensure accurate announcements.
