...
## 2024-09-14 - Add contextual ARIA labels to emoji buttons
**Learning:** In EmojiPickerModal and MagnetCommentDrawer, emoji buttons were relying solely on `title` attributes or image `alt` text (e.g. "Smile"). While `alt` text provides an accessible name, it describes the image itself rather than the action. Screen reader users would hear "Smile button", which doesn't clearly communicate the action of selecting or reacting.
**Action:** When creating icon-only buttons for specific actions (like picking an emoji or reacting), always provide an explicit `aria-label` that describes the *action* (e.g., `aria-label="Select Smile"` or `aria-label="React with Smile"`), rather than relying on the image's `alt` text which only describes the visual content. For toggleable reactions, ensure the label reflects the current state (e.g., `aria-label={hasReacted ? 'Remove reaction' : 'React with...'}`).
