...
## 2024-07-24 - Accessible Custom Dropdowns
**Learning:** Custom dropdown menus built with `<div>` and `<button>` elements are not recognized as select inputs by screen readers.
**Action:** When creating custom dropdowns, always add `aria-haspopup="listbox"` and `aria-expanded` to the trigger button, `role="listbox"` to the panel container, and `role="option"` with `aria-selected` to each option element.
