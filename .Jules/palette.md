...
## 2024-08-14 - Custom Dropdown Accessibility
**Learning:** Custom dropdown menus built with `div` and `button` elements instead of native `<select>` tags lack inherent accessibility. Screen readers do not know they function as a list of options.
**Action:** When building custom select/dropdown components, always ensure proper screen reader support by adding `aria-haspopup="listbox"` and `aria-expanded={isOpen}` to the trigger button, `role="listbox"` to the panel container, and `role="option"` with `aria-selected={isSelected}` to each option element.
