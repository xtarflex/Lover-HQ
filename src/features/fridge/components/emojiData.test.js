...
describe('emojiData', () => {
  ...
  describe('Maps', () => {
    it('should have a Map for looking up emojis by ID', () => {
      expect(ANIMATED_EMOJIS_BY_ID instanceof Map).toBe(true);
      expect(ANIMATED_EMOJIS_BY_ID.get('heart').id).toBe('heart');
    });

    it('should have a Map for looking up emojis by char', () => {
      expect(ANIMATED_EMOJIS_BY_CHAR instanceof Map).toBe(true);
      expect(ANIMATED_EMOJIS_BY_CHAR.get('❤️').id).toBe('heart');
    });
  });
});
